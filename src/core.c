...
const char* rebx_version_str = "3.3.0";         // **VERSIONLINE** This line gets updated automatically. Do not edit manually.
...
void rebx_register_default_params(struct rebx_extras* rebx){
    ...
    rebx_register_param(rebx, "dadt_1km", REBX_TYPE_DOUBLE);
    rebx_register_param(rebx, "obliquity", REBX_TYPE_DOUBLE);
}
...
struct rebx_force* rebx_load_force(struct rebx_extras* const rebx, const char* name){
    ...
    else if (strcmp(name, "yarko_dadt_force") == 0){
        force->update_accelerations = rebx_yarko_dadt_force;
        force->force_type = REBX_FORCE_VEL;
    }
    else{
        char str[300];
